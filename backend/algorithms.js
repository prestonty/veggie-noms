...
function macroRatio(calsBurn, type,weight){
    ...
    type === 0? (PR = 0.30, CR = 0.40, FR = 0.30): (type === 1? (PR = 0.30, CR = 0.50, FR = 0.20):(PR = 0.40, CR = 0.30, FR = 0.30)); 
    type === 1?(calsBurn += (1.0*weight*weight)/(40.0)):(type === 2? calsBurn -= (1.0*weight*weight)/(40.0): calsBurn = calsBurn);
    return [Math.round(PR*calsBurn)/4,Math.round(CR*calsBurn)/4,Math.round(FR*calsBurn)/9];
} 
