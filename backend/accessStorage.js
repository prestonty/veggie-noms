...
const { getMeasurements } = require("./algorithms.js");
...
async function main() {
  ...
  try {
    ...
    proteins = await findItemsFromProtein(
      client,
      (proteinAmount / 3) * (0.2 + Math.random() * 0.3)
    );
    ...
    proteins = await findItemsFromProtein(
      client,
      proteinAmount / 3 - foods[0].protein
    );
    ...
    carbs = await findItemsFromCarbs(
      client,
      (carbAmount / 2) * (0.2 + Math.random() * 0.3)
    );
    ...
    fats = await findItemsFromFat(
      client,
      fatAmount * (0.2 + Math.random() * 0.3)
    );
